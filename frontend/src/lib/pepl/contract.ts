// VENDORED — verbatim copy of backend/src/types.ts (the zod schemas are the law).
// The frontend deploys on its own (Vercel build sees only frontend/), so it can't
// reach ../../../../backend at build time. We copy the contract here and consume it
// type-only from ./types.ts, so zod is fully erased from the browser bundle.
// pepl: keep in sync with backend/src/types.ts on any contract change. Upgrade path:
// extract a shared @pepl/contract workspace package if a third consumer appears.
import { z } from "zod";
