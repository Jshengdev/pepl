...
| Window | Owns | Brief |
|---|---|---|
| **1 · Tech & Porting** *(this repo's current window)* | what to port / not port from doubles, what to keep as context, backend architecture + CONTRACTS, the ½-LOC shrink | [tech-porting.md](./tech-porting.md) |
...
```
Window 1 (tech)  ──CONTRACTS + port manifest──────────▶ Window 3 (build loop)
...
| doubles | `/Users/johnnysheng/code/doubles` | the engine to port → Window 1 |
...
- Ponytail: ~½ doubles' LOC; fewest deps / files.
